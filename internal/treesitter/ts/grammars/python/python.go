// Package python provides the Python grammar via the official tree-sitter
// binding, wrapped as a ts.Language for the official adapter. Sibling of the
// Phase-0 golang/ package (B2 cutover Part A, ADR 0023, #5418): migrating a
// language is a package like this one plus a migratedLanguages line in
// adapters_official.go.
//
// ABI pin. The grammar is pinned to tree-sitter-python v0.23.6 against runtime
...
// but SIGSEGVs at RootNode (ADR 0023 §6). Do not bump past ABI 14 without the
...
// Language returns the Python grammar as a ts.Language bound to the official adapter.
func Language() ts.Language {
	return official.WrapLanguage(tsofficial.NewLanguage(tspython.Language()))
}
