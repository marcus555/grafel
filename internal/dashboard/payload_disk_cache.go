...
func (c *diskPayloadCache) Set(key, sourceVersion string, entry *payloadEntry) error {
	...
	data := make([]byte, 0, diskPayloadHeader+len(key)+len(entry.etag)+len(entry.body))
	data = append(data, diskPayloadMagic...)
	data = binary.LittleEndian.AppendUint32(data, uint32(len(key)))
	data = binary.LittleEndian.AppendUint32(data, uint32(len(entry.etag)))
	data = binary.LittleEndian.AppendUint64(data, uint64(len(entry.body)))
	data = append(data, sum[:]...)
	data = append(data, key...)
	data = append(data, entry.etag...)
