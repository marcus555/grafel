...
| Endpoint pagination posture | 🟢 `partial` | `2026-06-04` | 3628 | `internal/engine/http_endpoint_pagination.go`<br>`internal/engine/http_endpoint_pagination_pybody_3872_test.go`<br>`internal/engine/http_endpoint_synthesis.go` | #3872 (epic #3628): NOW PARTIAL. The Python separate-handler body locator (findPyHandlerBody, mirroring Go findGoHandlerBody) resolves the out-of-line aiohttp handler (app.router.add_get("/items", list_items) / add_route / @routes.get) via the endpoint's source_handler ref and scans its real coroutine body with the existing request.query.get("limit") sniffer. A limit+offset pair → offset, a cursor token → cursor; a lone limit stays honest-missing (ambiguous). HONEST-PARTIAL: dynamically-named reads (request.query.get(var)) and pagination params named only in middleware are still unreached. |
