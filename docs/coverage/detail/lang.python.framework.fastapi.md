...
| Rate limit stamping | ✅ `full` | `2026-06-02` | [link](https://github.com/cajasmota/grafel/issues/3778) | `internal/custom/python/fastapi.go`<br>`internal/custom/python/rate_limit_endpoint.go`<br>`internal/custom/python/rate_limit_endpoint_test.go` | slowapi @limiter.limit("5/minute") stamps rate_limited/rate_limit/rate_limit_source on the route op. DRF @throttle_classes resolver (rate from a co-located custom throttle's rate attr; settings-driven built-ins → honest-partial) shared via resolvePyEndpointRateLimit. |
