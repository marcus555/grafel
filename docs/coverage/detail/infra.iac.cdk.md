...
| Iac resource property extraction | — `not_applicable` | — | — | — | CDK emitResource stamps only construct_type/logical_id/resource_category; the props/kwargs body (cdkConstructPropsRe Pass 5) is mined solely for props_ref DEPENDS_ON edges and never stamps a scalar property value onto the resource entity (cdk_edges.go:281-294,391-409). No typed config properties extracted — honest-missing. |
