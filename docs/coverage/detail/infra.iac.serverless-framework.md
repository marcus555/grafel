...
| Iac resource property extraction | 🟢 `partial` | `2026-06-03` | [link](https://github.com/cajasmota/archigraph/issues/4199) | `internal/engine/serverless_framework_edges.go` | Each function entity is stamped with its typed function-config properties: handler, runtime, region, service (parseProviderBlock + parseFunctionBody -> fnProps, serverless_framework_edges.go:144-161) — genuine per-function settings on the entity. memory/timeout/environment are not parsed/stamped (slsFunction struct has only name/handler/events, serverless_framework_parse.go:24-28), so partial. |
