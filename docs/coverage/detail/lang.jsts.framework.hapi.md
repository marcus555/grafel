...
| Endpoint pagination posture | 🟢 `partial` | `2026-06-03` | 3872 | `internal/engine/http_endpoint_pagination.go`<br>`internal/engine/http_endpoint_pagination_patterns.go`<br>`internal/engine/http_endpoint_pagination_test.go`<br>`internal/engine/http_endpoint_synthesis.go` | #3628/#3872: applyEndpointPagination stamps paginated/pagination_style/pagination_params on the hapi endpoint via jsPaginationVerdict — limit+offset/page/cursor read from req.query/request.query in the handler body (classifyParamShape). Partial: query-read shape only (no ORM/cursor-class signal like express). Honest-partial: a lone limit is ambiguous and left unstamped. |
| Endpoint response codes | ✅ `full` | `2026-06-04` | 3818 | `internal/engine/http_endpoint_response_codes.go`<br>`internal/engine/http_endpoint_response_codes_test.go`<br>`internal/engine/http_endpoint_synthesis.go` | #3818: response_codes + success_code + response_codes_source via applyEndpointResponseCodes. Hapi signals: h.response(x).code(NNN) chained status literal in route handler. Honest-partial: .code(dynamicVar) skipped; no literal -> absent. |
| Endpoint synthesis | ✅ `full` | `2026-05-28` | [link](https://github.com/cajasmota/grafel/issues/2851) | `internal/engine/http_endpoint_jsts_backend.go`<br>`internal/engine/rules/javascript_typescript/frameworks/hapi.yaml`<br>`testdata/fixtures/typescript/hapi_routes.ts` | — |
| Handler attribution | ✅ `full` | `2026-05-28` | [link](https://github.com/cajasmota/grafel/issues/2851) | `internal/engine/http_endpoint_jsts_backend.go`<br>`internal/engine/rules/javascript_typescript/frameworks/hapi.yaml`<br>`testdata/fixtures/typescript/hapi_routes.ts` | — |
...
| Capability | Status | Verified at | Issue | Cites | Notes |
|------------|--------|-------------|-------|-------|-------|
| Auth coverage | ✅ `full` | `2026-05-28` | — | `cmd/grafel/audit2852_jsauth_test.go`<br>`internal/engine/http_endpoint_jsts_auth.go`<br>`internal/engine/http_endpoint_jsts_auth_test.go`<br>`testdata/fixtures/typescript/hapi_auth.ts` | — |
...
| Trace extraction | ✅ `full` | `2026-06-24` | [link](https://github.com/cajasmota/grafel/issues/5500) | `internal/extractors/javascript/observability.go`<br>`internal/extractors/javascript/tracing.go`<br>`internal/patterns/observability_jsts_extractor.go`<br>`internal/patterns/observability_jsts_extractor_test.go` | OpenTelemetry span-creation now emits the actual span -> INSTRUMENTS edge (enclosing fn -> span:<name> stub), at parity with the Python lane, not just presence. #5500 (epic #5479): AST pass over fn/method/arrow bodies recognises tracer.startSpan / tracer.startActiveSpan, inline trace.getTracer(...).startSpan(...), @vercel/otel registerOTel(service), and manual context.with(trace.setSpan(...)) scopes, ATTRIBUTION-GATED on an @opentelemetry/* or @vercel/otel import OR a tracer-like receiver so an unrelated .startSpan( is not matched. Sentry/dd-trace span+transaction sites likewise emit INSTRUMENTS via observability.go. The regex observability_jsts_extractor.go still contributes the presence SCOPE.Config signal. Honest-partial rule: dynamic/variable span names emit traced=true without a fabricated name. |
