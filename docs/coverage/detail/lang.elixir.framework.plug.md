...
|------------|--------|-------------|-------|-------|-------|
| Endpoint deprecation versioning | ✅ `full` | `2026-06-03` | 4146 | `internal/custom/elixir/endpoint_deprecation.go`<br>`internal/custom/elixir/endpoint_deprecation_test.go` | #4146 (child of #3628) Elixir port: same custom-extractor-stage deprecation/api_version pass as phoenix (internal/custom/elixir/endpoint_deprecation.go). A raw Plug.Router file (use Plug.Router / Plug.Conn) is labelled framework=plug by detectElixirDepFramework. The @deprecated "<msg>" attribute above a Plug route handler (a `get "/v1/..." do` block or a def taking conn), the @doc deprecated: form, a # DEPRECATED banner, and a put_resp_header(conn,"sunset"|"deprecation",...) RFC 8594 header credit deprecated=true with honest-partial deprecated_since/deprecated_replacement; api_version is path-derived from a /api/vN or /vN route literal. Identical property contract to the flagship. Value-asserted TestElixirDep_PlugFramework (framework=plug, api_version=1); shared negatives TestElixirDep_VersionlessNoApiVersion + TestElixirDep_NonRouteDeprecatedUnaffected. Live-firing confirmed via RunCustomExtractors. |
