...
| Endpoint synthesis | ✅ `full` | `2026-07-03` | — | `internal/engine/aspnet_core_routes.go`<br>`internal/engine/http_endpoint_csharp_abp.go`<br>`internal/engine/http_endpoint_csharp_abp_test.go`<br>`internal/engine/http_endpoint_synthesis.go`<br>`internal/engine/rules/csharp/frameworks/asp_net_core.yaml` | ASP.NET Core attribute routing and YAML rule coverage remain full; ABP Framework conventional controllers are now synthesized from *AppService/ApplicationService/CrudAppService methods into canonical http_endpoint_definition entities. |
| Handler attribution | ✅ `full` | `2026-07-03` | — | `internal/engine/aspnet_core_routes.go`<br>`internal/engine/http_endpoint_csharp_abp.go`<br>`internal/engine/http_endpoint_csharp_abp_test.go` | ASP.NET endpoints carry source_handler attribution to SCOPE.Operation handlers; ABP conventional AppService endpoints attribute to <AppService>.<Method> so the shared resolver can bridge endpoint to method. |
| Route extraction | ✅ `full` | `2026-07-03` | — | `internal/custom/csharp/aspnet_core.go`<br>`internal/engine/aspnet_core_routes.go`<br>`internal/engine/http_endpoint_csharp_abp.go`<br>`internal/engine/http_endpoint_csharp_abp_test.go` | Minimal-API app.MapGet/Post/Put/Delete route path strings extracted via reAspNetMinimalAPI; attribute routes via reAspNetHTTPMethod; ABP conventional AppService routes synthesized under /api/app/<service> with method-name verb/action conventions. |
