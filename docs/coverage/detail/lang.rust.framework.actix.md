...
| Route extraction | ✅ `full` | `2026-06-12` | — | `internal/custom/rust/actix_web.go`<br>`internal/custom/rust/extractors_test.go`<br>`internal/custom/rust/helpers.go` | Extracts attribute-macro and manual web::get().to() routes; normalises path params; composes web::scope() prefix onto manual routes. #4921: WebSocket handlers ARE recovered too — reActixWebSocket (ws::WebsocketContext<H>) synthesises a SCOPE.Operation/websocket entity per handler (provenance INFERRED_FROM_ACTIX_WEBSOCKET, handler_name stamped); the dedicated websocket_route_extraction cell (#4965) now carries the richer routed WS contract. |
| Websocket route extraction | ✅ `full` | `2026-06-14` | 4965 | `internal/custom/rust/websocket_routes.go`<br>`internal/custom/rust/websocket_routes_test.go` | #4965 dedicated websocket_route_extraction (new http_backend taxonomy key; previously actix_web.go step 7 emitted a bare SCOPE.Operation/websocket from ws::WebsocketContext<H> with no upgrade-mechanism/contract). custom_rust_websocket_routes recovers all three actix WS idioms: ws::start(Actor{...}) (actix-web-actors), ws::WebsocketContext<H>, and actix_ws::handle(&req, stream) (the newer actix-ws crate), stamping handler_name (actor struct), websocket=true, upgrade_mechanism (ws::start | WebsocketContext | actix_ws::handle). route_path is honestly OMITTED — actix mounts the upgrade on a normal HTTP route elsewhere, not on the upgrade idiom. Value-asserted TestRustWS_ActixActorStart (WS MyWs (actix) -> handler_name=MyWs, upgrade_mechanism=ws::start), TestRustWS_ActixWsHandle (actix_ws::handle). Honest-partial + wrong-lang/no-match no-op shared with the axum tests. |
