...
| Capability | Status | Verified at | Issue | Cites | Notes |
|------------|--------|-------------|-------|-------|-------|
| DTO extraction | ✅ `full` | `2026-06-02` | — | `internal/custom/javascript/express.go`<br>`internal/custom/javascript/reqresp_dto_test.go` | express.go now emits traversable endpoint→DTO edges for TypeScript handlers (#3629/#3607): a route SCOPE.Operation carries ACCEPTS_INPUT → request DTO and RETURNS → response DTO when the handler is typed via the Express Request<P, ResBody, ReqBody> generic (3rd arg = request body, 2nd = response body) or Response<ResBody>, ToID=Class:<Name>. Untyped req.body handlers and plain JavaScript carry NO edge (honest-partial) — Express provides no type signal there. Tests: TestExpressReqResp_TypedRequestBody (Request<{},UserDto,CreateUserDto> → ACCEPTS_INPUT Class:CreateUserDto + RETURNS Class:UserDto), TestExpressReqResp_TypedResponse (Response<ProfileDto> → RETURNS Class:ProfileDto), negatives TestExpressReqResp_UntypedNoEdge / TestExpressReqResp_JavaScriptNoEdge. |
...
| Feature flag gating | 🔴 `missing` | `2026-06-02` | feature_flag_gating:#3706-not-yet-extracted | `internal/engine/feature_flag_edges.go`<br>`internal/engine/feature_flag_edges_test.go`<br>`internal/engine/orm_queries.go` | flag-check call sites -> feature:<key> + GATED_BY edge (LaunchDarkly/Unleash/OpenFeature/Flipper/Flagsmith) |
