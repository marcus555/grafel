...
| Iac resource property extraction | — `not_applicable` | — | — | — | extractResourceBlock stamps resource_type/label/resource_category + iteration meta-args (count/for_each) only; the scalar resource block attributes (instance_type = "…", ami, …) are never stamped onto the entity (hcl/extractor.go:206-249, terraform_deep.go:74-97). Properties body is read only for depends_on/interpolation refs. Honest-missing. |
