...
| Iac resource property extraction | — `not_applicable` | — | — | — | Pulumi emitResource stamps construct_type/resource_category/logical_id only; the typed args body is scanned solely for output-ref DEPENDS_ON edges (applyPulumiArgEdges), discarding scalar arg values (pulumi_edges.go:242-254,373-411). No typed config properties stamped — honest-missing. |
