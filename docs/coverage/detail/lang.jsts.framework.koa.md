...
| Endpoint pagination posture | 🟢 `partial` | `2026-06-03` | 3872 | `internal/engine/http_endpoint_pagination.go`<br>`internal/engine/http_endpoint_pagination_patterns.go`<br>`internal/engine/http_endpoint_pagination_test.go`<br>`internal/engine/http_endpoint_synthesis.go` | #3628/#3872: applyEndpointPagination stamps paginated/pagination_style/pagination_params on the koa endpoint via jsPaginationVerdict — limit+offset/page/cursor read from req.query/request.query in the handler body (classifyParamShape). Partial: query-read shape only (no ORM/cursor-class signal like express). Honest-partial: a lone limit is ambiguous and left unstamped. |
| Endpoint response codes | ✅ `full` | `2026-06-04` | 3818 | `internal/engine/http_endpoint_response_codes.go`<br>`internal/engine/http_endpoint_response_codes_test.go`<br>`internal/engine/http_endpoint_synthesis.go` | #3818: response_codes + success_code + response_codes_source via applyEndpointResponseCodes. Koa signals: ctx.status = NNN assignment literal in handler body. Honest-partial: ctx.status = dynamicVar skipped; no literal -> absent. |
