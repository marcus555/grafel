...
| Endpoint pagination posture | ✅ `full` | `2026-06-14` | 5101 | `internal/custom/rust/endpoint_pagination.go`<br>`internal/custom/rust/endpoint_pagination_test.go` | #5101 (child of #5019/#3628) closes the last handler-less Rust HTTP framework for pagination posture, mirroring the response_codes hyper recovery. paginated/pagination_style/pagination_params/pagination_source are stamped at the SOURCE by re-emitting the SCOPE.Operation/endpoint op so it merges onto the producer route op by Name (the engine applyEndpointPagination pass is gated on http_endpoint_definition and cannot reach Rust SCOPE.Operation custom entities). hyper routes are `match (req.method(), path) { (&Method::VERB, "/p") => handler(req) }` arms: the arm RHS NAMES a handler resolved via the shared handler->verdict map (named-handler form, rustHyperRespArmRe), OR the arm is an INLINE block whose body carries the pagination idiom directly (inline form, rustHyperRespInlineArmRe — body window clipped at the next arm / `_ =>` wildcard so a sibling arm's signal never bleeds in). Reuses the shared honest classifier: typed Query<Struct> fields, diesel/sqlx `.limit().offset()` PAIR (offset), sea_orm `.paginate()`/`Paginator` (page), literal query reads; a lone limit-like param or lone `.limit()` is ambiguous and NOT stamped. Honest-partial: a match arm whose handler/body resolves no clear pagination shape is left to the producer (no fabricated pagination). Value-asserted: TestRustPagination_HyperNamedHandler (GET /users -> offset limit,offset; POST not stamped), _HyperInlineArm (inline Query<Page> -> page, clipped at sibling /health), _HyperNoPaginationNotStamped (negative), _HyperWrongLanguageNoOp (wrong-language no-op). |
| Endpoint response codes | ✅ `full` | `2026-06-14` | 5018 | `internal/custom/rust/endpoint_response_codes.go`<br>`internal/custom/rust/endpoint_response_codes_test.go` | #5018 (child of #3628, follow-up to #4965) closes the last Rust HTTP framework: response_codes/success_code/response_codes_source stamped at the SOURCE by re-emitting the SCOPE.Operation/endpoint op so it merges onto the producer route op by Name (the engine applyEndpointResponseCodes pass is gated on http_endpoint_definition and cannot reach Rust SCOPE.Operation custom entities). hyper routes are `match (req.method(), path) { (&Method::VERB, "/p") => handler(req) }` arms: the arm RHS NAMES a handler resolved via the shared handler->verdict map (named-handler form, rustHyperRespArmRe), OR the arm is an INLINE block whose body carries the status idiom directly (inline form, rustHyperRespInlineArmRe — body window clipped at the next arm / `_ =>` wildcard so a sibling arm's codes never bleed in). Shared StatusCode/Status name table + numeric constructors + Response::builder().status() setters. Honest-partial: a match arm whose handler/body resolves no literal status is left to the producer (no fabricated 200). Value-asserted: TestRustRespCodes_HyperNamedHandler (GET/POST /users -> 200/201), _HyperInlineArm (inline arm body, clipped at sibling), _HyperNoStatusNotStamped (negative), _HyperWrongLanguageNoOp (wrong-language no-op). |
...
| Request shape extraction | 🟢 `partial` | `2026-05-28` | [link](https://github.com/cajasmota/grafel/issues/2771) | `internal/links/payload_drift.go`<br>`internal/mcp/payload_drift_tool.go`<br>`internal/substrate/payload_shapes.go`<br>`internal/substrate/payload_shapes_rust.go` | — |
| Request sink dataflow | 🔴 `missing` | — | 3740 | — | — |
| Response shape extraction | 🟢 `partial` | `2026-05-28` | [link](https://github.com/cajasmota/grafel/issues/2771) | `internal/links/payload_drift.go`<br>`internal/mcp/payload_drift_tool.go`<br>`internal/substrate/payload_shapes.go`<br>`internal/substrate/payload_shapes_rust.go` | — |
| Sanitizer recognition | 🟢 `partial` | `2026-05-28` | — | `internal/links/taint_flow.go`<br>`internal/substrate/taint_sites_rust.go` | — |
| Schema drift detection | 🟢 `partial` | `2026-05-28` | [link](https://github.com/cajasmota/grafel/issues/2771) | `internal/links/payload_drift.go`<br>`internal/mcp/payload_drift_tool.go`<br>`internal/substrate/payload_shapes.go`<br>`internal/substrate/payload_shapes_rust.go` | — |
