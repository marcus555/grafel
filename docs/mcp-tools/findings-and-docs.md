...
Output stored at `~/.grafel/findings/<group>/`.
...
Agent pattern store (ADR-0018) — distinct from the indexer-extracted [`grafel_graph_patterns`](cross-cutting-analysis.md#grafel_graph_patterns).
...
List canonical doc files for a group under `~/.grafel/docs/<group>/`.
