...
=======
| flask-realworld | python | 43 | 7.49% (2026-05-19, extends-kind-disamb #525; was 7.60%) | extends-kind-disamb #525 | resolver tier-1 real-vs-SCOPE preference for EXTENDS/IMPLEMENTS on top of python-django-w4 (django/DRF/flask/SQLA framework base-class allowlist + Wave-4 pythonBareNames + knownExternalPackages + super.X dynamic regex). 2 EXTENDS edges (`SurrogatePK`-family base classes that collide between real Component import and same-file SCOPE.Component placeholder) now bind. Residual: SQLA `Query.first()` (ruby/swift collision blocks lang gate), generic verbs `cls`/`pop`/`items`/`remove`/`append` (too collision-prone), `User.save`/`UserProfile.save` dotted-receiver class member access (extractor receiver-binding) | at-bar | dotted-receiver class member binding + ruby-collision-free `first` route (chain-fix) |
| click | python | 138 | 6.45% (2026-05-19, extends-kind-disamb #525 confirmed flat vs python-django-w4) | extends-kind-disamb #525 | resolver tier-1 real-vs-SCOPE preference confirmed flat on click (no Component/SCOPE.Component same-file collisions on EXTENDS). Shared pythonBareNames + knownExternalPackages additions reach click via cumulative wins | at-bar | next python wave for ship-gate gap |
| django-realworld | python | 34 | 6.42% (2026-05-19, extends-kind-disamb #525; was 7.83%) | extends-kind-disamb #525 | resolver tier-1 real-vs-SCOPE preference for EXTENDS/IMPLEMENTS on top of python-django-w4 (django/DRF framework base-class allowlist + Wave-4 pythonBareNames + super.X dynamic regex + bare-kind-prefixed lang="" allowlist fallback). 15 EXTENDS edges (`TimestampedModel`, `AbstractBaseUser`, `PermissionsMixin`, `ConduitJSONRenderer` base classes that collide between real Component import and same-file SCOPE.Component placeholder) now bind. Residual: `*.serializer_class` / `*.get_queryset` dotted ViewSet attribute access (extractor receiver-binding), `Route:conduit.apps.X.urls` cross-language URLConf includes, generic verbs (`pop`/`items`/`now`/`choice`/`timedelta`) | at-bar | DRF ViewSet attribute receiver-binding (chain-fix) |
| pandas | python | 197 | 10.11% (2026-05-19, extends-kind-disamb #525 confirmed flat vs python-django-w4) | extends-kind-disamb #525 | resolver tier-1 real-vs-SCOPE preference confirmed flat on pandas. Wave-4 numpy/pandas bare-names (asarray=196/astype=195/is_list_like=145/find_stack_level=109/arange=80/zeros=74/Timestamp/Timedelta/AbstractMethodError/etc.) — top stubs accumulated 80-200 hits each pre-fix | structural | further pandas wave needed: `view`/`func`/`cls`/`equals`/`reindex`/`op` collision-prone, dotted `type._simple_new` receiver-binding |
>>>>>>> e9e3dc7 (fix(resolve): prefer Component over SCOPE.Component for EXTENDS/IMPLEMENTS (#525))
