...
| 3 | `prompts/03-with-mcp-run.md` | Answer the **same** questions using archigraph MCP tools only. Same metrics. Persist as `with-mcp.json`. **Runs SECOND** — fresh context that has not seen Phase 2's transcript. See `prompts/03a-rpc-capture.md` for the daemon RPC capture protocol. |
