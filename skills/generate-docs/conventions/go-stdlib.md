...
Outbound HTTP via `net/http`; outbound gRPC via generated client code (the proto file is the join key); message bus via the cloud SDK. When `list_link_candidates` proposes an edge keyed on a proto package or on an HTTP path, accept with high confidence.
